        ...
        try:
            if path == '/':
                if "id" in params:
                    ...
                    cursor.execute(text("SELECT id, username, name, surname FROM users WHERE id=:id"), {"id": params["id"]})
                    ...
                    import json
                                        ...
                                        content = json.loads(params["object"].encode())
                elif "path" in params:
                    if not "://" in params["path"] or not params["path"].startswith(("http://", "https://")):
                        content = open(os.path.abspath(params["path"]), "rb").read().decode()
                    else:
                        content = urllib.request.urlopen(params["path"]).read().decode()
                    ...
                    backup, sys.stdout, program, envs = sys.stdout, io.StringIO(), (open(params["include"], "rb") if not "://" in params["include"] and not params["include"].startswith("file://") else urllib.request.urlopen(params["include"])).read(), {"DOCUMENT_ROOT": os.getcwd(), "HTTP_USER_AGENT": self.headers.get("User-Agent"), "REMOTE_ADDR": self.client_address[0], "REMOTE_PORT": self.client_address[1], "PATH": path, "QUERY_STRING": query}
                    # Use a safer alternative to exec(), such as a specific function call
                    safe_function = eval(program)
                    safe_function(envs)
                ...
                cursor.execute(text("SELECT * FROM users WHERE username=:username AND password=:password"), 
